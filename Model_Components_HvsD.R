...
correlate_components_hvd <- function(eigenvectors, metadata, outputdir, type_receptor, metahealth, run_all){
	eigenvectors <- read.delim(eigenvectors, sep="\t", header=TRUE)
	outputdir <- paste0(outputdir, "TensorDecomposition/")
	...
	plots$module <- gsub("_" ," ", 	plots$module)
	plots$module <- str_wrap(plots$module, 15)
	plots$module <- factor(plots$module)
	pdf(paste0(outputdir,"/HvsD_Component_Scatter.pdf"), width=13, height=13)
	...
	for(i in 2:(length(colnames(eigenvectors))-7)){
		...
		################################
		nameuse <- colnames(eigenvectors)[i]
		nameuse <- gsub("_" ," ", 	nameuse)
		nameuse <- str_wrap(nameuse, 15)
	...
	vif_all_g <- vif_all_g[vif_all_g$V1 ==vif_all_g$V1[1],]
	...
	pdf(paste0(outputdir, "/HvsD_AdjModel_Residual_Component_Plot.pdf"), width=(dims+5), height=dims)
	...
	p_hd$Module <- factor(p_hd$Module)
	...
	write.table(p_hd, paste0(outputdir, "/HvsD_INTERACTION_ANALYSIS_SUMMARY.txt"), sep="\t")
	...
	if(interaction_pres =="YES"){
		...
		p_hd_extended_timepoint$MODULE <- factor(p_hd_extended_timepoint$MODULE)		
		...
		write.table(p_hd_extended_timepoint, paste0(outputdir, "/HvsD_INTERACTION_ANALYSIS_timepoint_", annot, ".txt"), sep="\t")
		## Lets remove the direction for ns effect
		...
		p_hd_extended_hvsd$MODULE <- factor(p_hd_extended_hvsd$MODULE)
		## Now we want to merge with behaviour 
		...
		write.table(p_hd_extended_hvsd, paste0(outputdir, "/HvsD_INTERACTION_ANALYSIS_hvsd_", annot, ".txt"), sep="\t")
		## Lets remove the direction for ns effect
		...
		pdf(paste0(outputdir, "/HvsD_SimpleEffects_Statistics_", annot, ".pdf"), width=12, height=14)
