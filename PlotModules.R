...
plot_eigenvectors <- function(eigenvectors, meta_data1, productivity, outputdir, type_use, subsampled_deptha){
	...
	mat_eigenvectors <- mat_eigenvectors[!rownames(mat_eigenvectors) %like% "JR",]
	
	mat_eigenvectors$DISEASE[mat_eigenvectors$sample %like% "HV"] <- "HEALTH"
	mat_eigenvectors$DISEASE[mat_eigenvectors$sample %like% "JR_HD_T0_POSITIVE"] <- "TECHNICAL"
	mat_eigenvectors$DISEASE[mat_eigenvectors$sample %like% "JR1795_1003_"] <- "TECHNICAL"
	...
	pdf(paste0(outputdir, "Plots/Clustered_Tensor_Component_byDAY_", type_use, "_", subsampled_deptha, "_", iso_type,".pdf"), height=7.5, width=7.5)
	for(i in 1:(length(colnames(mat_eigenvectors))-3)){
		...
		p <- ggplot(mat_eigenvectors, aes_string(x="DAY", y=sym(coluse))) +geom_boxplot(aes(fill=DISEASE)) + geom_point(aes( group=DISEASE), position=position_dodge(width=0.75),)+theme_bw(base_size = 6)+facet_wrap(~DISEASE) + stat_compare_means(comparisons =my_comparisons, label = "p.signif", exact=FALSE)
		...
		q <- ggplot(mat_eigenvectors, aes_string(x="DISEASE", y=sym(coluse))) +geom_boxplot(aes(fill=DISEASE)) + geom_point(aes( group=DAY), position=position_dodge(width=0.75),)+theme_bw(base_size = 6)+facet_wrap(~DAY) + stat_compare_means(comparisons =my_comparisons, label = "p.signif", exact=FALSE) 
		plot(plot_grid(p, q, ncol=1))
	...
	### Save eigenvectors
	write.table(mat_eigenvectors, paste0(outputdir, "Clustered_Tensor_Component_", type_use, "_", iso_type, ".txt"), sep='\t')
	print("DONE")
	...
	if(file.exists(meta_data1)){
		...
		rownames(mat_eigenvectors) <- sub("_productive", "", rownames(mat_eigenvectors))
		rownames(mat_eigenvectors) <- sub("_unproductive", "", rownames(mat_eigenvectors))
		...
		pdf(paste0(outputdir, "Plots/Clustered_Tensor_Component_byVariables_", type_use, "_", subsampled_deptha, "_", iso_type,".pdf"), height=10, width=10)
		for(i in 1:(length(colnames(mat_eigenvectors))-3)){
			...
			p <- ggplot(mat_eigenvectors_new, aes_string(x="DAY", y=sym(coluse))) +geom_boxplot(alpha=0.5, aes(fill=DISEASE)) +theme_bw(base_size = 6)+ facet_wrap(~DISEASE) + stat_compare_means(comparisons =my_comparisons, label = "p.signif", exact=FALSE) +guides(fill=FALSE)
			##############################
			...
			r <- ggplot(mat_eigenvectors_new, aes_string(x="outcome", y=sym(coluse))) +geom_boxplot(alpha=0.5, aes(fill=outcome)) +theme_bw(base_size = 6)+ facet_wrap(~DAY) + stat_compare_means(comparisons =my_comparisons, label = "p.signif", exact=FALSE) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+guides(fill=FALSE)
			##############################
			...
			s <- ggplot(mat_eigenvectors_new, aes_string(x="SRS", y=sym(coluse))) +geom_boxplot(alpha=0.5, aes(fill=SRS)) +theme_bw(base_size = 6)+ facet_wrap(~DAY) + stat_compare_means(comparisons =my_comparisons, label = "p.signif", exact=FALSE) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+guides(fill=FALSE)
			##############################
			...
			t <- ggplot(mat_eigenvectors_new, aes_string(x="Sex", y=sym(coluse))) +geom_boxplot(alpha=0.5, aes(fill=Sex)) +theme_bw(base_size = 6)+ facet_wrap(~DAY) + stat_compare_means(comparisons =my_comparisons, label = "p.signif", exact=FALSE) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+guides(fill=FALSE)
			##############################de
